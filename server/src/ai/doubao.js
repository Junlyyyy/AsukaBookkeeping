// 豆包大模型客户端 — 火山方舟（Volcano Ark）OpenAI 兼容接口
// 用于自然语言记账解析：把一句话/语音文本解析为结构化记账字段
// 配置：环境变量 ARK_API_KEY（必填，火山方舟 API Key）
//       环境变量 ARK_MODEL（可选，推理接入点 ID，如 ep-xxxx；默认 doubao-pro-32k）
// 未配置 ARK_API_KEY 时返回 null，由调用方降级到规则解析
...
const ARK_TIMEOUT = 15000;

/** 豆包是否已配置（有 API Key 即视为可用） */
 ...
 */
export async function parseWithDoubao(text) {
...
只输出一个 JSON 对象。`;
  ...
  try {
    ...
    const timer = setTimeout(() => ctrl.abort(), ARK_TIMEOUT);
    ...
    if (!res.ok) {
      console.error(`[doubao] HTTP ${res.status}: ${(await res.text()).slice(0, 200)}`);
      return null;
    }
    ...
    console.error(`[doubao] 解析失败: ${e.message}`);
    return null;
  }
}
